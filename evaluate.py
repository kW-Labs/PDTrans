...
def evaluate(model, autoencoder, test_loader, params, plot_num=None):
    ...
    '''
    data_Norm = np.load('data/elect/data_Norm.npy')
    CRPS = 0
    ...
    with torch.no_grad():
        ...
        for i, (test_batch, id_batch, labels) in enumerate(tqdm(test_loader)):
            ...
            mu = None;
            logvar = None
            distribution_mu, distribution_sigma = transformer.test(model, autoencoder, params, test_batch, id_batch, mu,
                                                                   logvar)
            ...
            scale_o = NormData[:, 1].unsqueeze(-1);
            mean_o = NormData[:, 0].unsqueeze(-1)
...
def save_model(model, filename="model_state.pth.tar"):
...
if __name__ == '__main__':
    ...
    model = transformer.EncoderDecoder(params=params, emb=emb, encoder=transformer.Encoder(params,
                                                                                           transformer.EncoderLayer(
                                                                                               params, c(attn), c(ff),
                                                                                               dropout=params.dropout)),
                                       decoder=transformer.Decoder(params,
                                                                   transformer.DecoderLayer(params, c(attn), c(attn),
                                                                                            c(ff),
                                                                                            dropout=params.dropout)),
                                       generator=ge)
    ...
    test_metrics = evaluate(model, autoencoder, test_loader, params, params.sampling)
    ...
    save_model(model, os.path.join(model_dir, "model_state.pth.tar"))